...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `PreventProjectDeletion` BEFORE DELETE ON `projects` FOR EACH ROW BEGIN
    DECLARE team_members_count INT;
   
    -- Check if there are team members assigned to the project
    SELECT COUNT(*)
    INTO team_members_count
    FROM TeamMembers
    WHERE ProjectID = OLD.ProjectID;
   
    -- If there are team members assigned, raise an error
    IF team_members_count > 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Cannot delete a project that still has assigned team members.';
    END IF;
END */;;
DELIMITER ;
